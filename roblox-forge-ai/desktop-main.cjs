...
function trusted(e){return !!win&&!win.isDestroyed()&&e?.sender===win.webContents;}
...
function startBridge(noxeryKey,autoDev=false,model="gpt-6-astra"){
 noxeryKey=String(noxeryKey||"").trim();
 ...
 bridge=spawn(process.execPath,[bridgePath,noxeryKey,autoDev?"1":"0","gpt-6-astra",String(model||"gpt-6-astra")],{env:{...process.env,ELECTRON_RUN_AS_NODE:"1",ELECTRON_NO_ASAR:"1"},stdio:["pipe","pipe","pipe"],windowsHide:false});
