...
local style = field("GAME STYLE","Studded",6)
...
local function save()
  ...
  plugin:SetSetting("style",style.Text)
end
...
local function load()
  ...
  style.Text=tostring(plugin:GetSetting("style") or "Studded")
end
load()
...
You must preserve the user-selected visual style across GUI, map, props, materials, lighting, VFX and asset choices. Never mix visual languages unless explicitly requested.
...
local function executePrompt(id,prompt,selectedStyle,styleContract)
...
local function poll()
  ...
  task.spawn(function()
    local ok,err=pcall(function()
      ...
      if r.command then executePrompt(tostring(r.command.id),tostring(r.command.prompt),tostring(r.command.style or style.Text),tostring(r.command.style_contract or "")) end
    end)
    if not ok then
